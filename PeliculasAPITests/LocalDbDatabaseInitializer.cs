    ...
    public class LocalDbDatabaseInitializer
    {
        ...
        public static ApplicationDbContext GetDbContextLocalDb(bool beginTransaction = true)
        {
            var options = new DbContextOptionsBuilder<ApplicationDbContext>()
                .UseSqlServer($"Data Source=.;Initial Catalog=PruebasDeIntegracion;Integrated Security=True; TrustServerCertificate=True",
...
#pragma warning disable CS8604 // Posible argumento de referencia nulo
        static string Filename => Path.Combine(
           Path.GetDirectoryName(
               path: typeof(LocalDbDatabaseInitializer).GetTypeInfo().Assembly.Location),
           $"{_dbName}.mdf");
